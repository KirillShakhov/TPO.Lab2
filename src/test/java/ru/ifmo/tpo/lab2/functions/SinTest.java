...
public class SinTest {
    ...
    public void shouldCalculateForPiDividedByTwoCicle() {
        ...
        for (int i = 0; i < 100; i+=2){
            assertEquals(1, sin.solve(Math.PI / 2 + (i * Math.PI), DEFAULT_PRECISION), DEFAULT_PRECISION);
    ...
    public void shouldCalculateForOne() {
        final Sin sin = new Sin();
        assertEquals(Double.valueOf("0.8414"), sin.solve(Double.valueOf(1), DEFAULT_PRECISION), DEFAULT_PRECISION);
    }

    @Test
    public void shouldCalculateForPeriodic() {
        final Sin sin = new Sin();
        assertEquals(Double.valueOf("0.09718"), sin.solve(Double.valueOf(-113), DEFAULT_PRECISION), DEFAULT_PRECISION);
    }
}
